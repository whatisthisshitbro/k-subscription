$.extend(Renderer, {
  assemblyTweets: function (tweets, timelineId) {
    ...
    var real=OptionsBackend.get('real_channel');
    var me=tweetManager.twitterBackend.username();
	
	if ((real.length>0)&&(timelineId.indexOf("search")<0)){
		var temp=[];
		for(var i=0;i<real.length;i++)
		{
			temp.push(real[i].noise)
		}
		checknoise=temp.toString().split(',');
		for(var i=0;i<tweets.length;++i) 
		{
			sender=tweets[i].user.screen_name;
			if(sender.toUpperCase()!=me.toUpperCase())
			{
				if (checknoise.indexOf(sender)<0)
