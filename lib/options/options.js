...
function loadValues(source) {
	...
	for(var i=0;i<ar.length;i++)
	{
		...
		{
			if(((ar[i].chnl!='Add one...')||(reals.indexOf(ar[i].chnl)!==-1))&&(typeof ar[i].chnl != 'undefined'))
				reals=reals+", "+ar[i].chnl;
		}
	}
	return reals;
}
...
function Options() {
	...
	this.load = function(forceDefault) {
		var _this = this;
		this.optionsMap=OptionsBackend.load();
		var realsA=_this.optionsMap['real_channel'];
		localStorage['sensitive_channels']=JSON.stringify(realsA);
		...
		$('textarea').each(function() {
			...
			if($this.attr('id')) {
				var reals='';
				var realsB=JSON.parse(localStorage["sensitive_channels"]);
				_this.optionsMap['real_channel']=realsB;
				reals=loadValues(_this.optionsMap['real_channel']);
				if((reals!='')&&(typeof reals != 'undefined'))
					$this.val(reals.replace(/"/g, ''));
			}
		});
	...
	this.save = function() {
		...
		if(!hasErrors) {
			...
			$('input,select,textarea').each(function() {
				var $this = $(this);
				var name = $this.attr('name');
				if((name)|| ($this.is('[id="real_channel"]'))){
					var converter = $this.attr('converter');
					var newValue = null;
					if(converter) {
					...
					if($this.is('[id="real_channel"]')){
						var reals=JSON.parse(localStorage["sensitive_channels"]);
						if(typeof reals != 'undefined')
						{	
							_this.optionsMap['real_channel']=reals;
						}
					}
				}
  				return true;
